...
// 			$this->db->where('status',$status);
	...
	{	 if($limit!=0){
		...
		$result=$this->db->where('subscription_validity_datetime>=',date('Y-m-d'))->where('registration_status',1)->order_by('id','desc')->get('student_registration');
	...
	else if($text!=0)
	{
		...
		$mobileNumber=$this->db->where('registration_status','1')->where('registered_mobile_no',$text)->order_by('id','desc')->get('student_registration');

		$studentName=$this->db->where('registration_status','1')->where('student_name',$text)->order_by('id','desc')->get('student_registration');

		$studentEmail=$this->db->where('registration_status','1')->where('registered_email',$text)->order_by('id','desc')->get('student_registration');
	...
	else
	{
		...
		$result=$this->db->where('registration_status','1')->order_by('id','desc')->get('student_registration');
		return $result->result();
	}
